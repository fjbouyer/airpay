class OrdersController < ApplicationController
  ...
  def new
    ...
    unless @reservation.nil?
      ...
      @reservation_price_per_person = @order.reservation.number_of_hour * @order.reservation.price_per_hour_cent_incl_vat
    end
  end
  ...
  def create
    ...
    if cart_infos["resa"]["reservation_id"] == params_reservation_id
      ...
      calculated_total_price_cent_incl_vat = 0
      price_of_reservation =  @reservation.number_of_hour *
                              @reservation.price_per_hour_cent_incl_vat *
      ...
      @order = Order.new( status: "En vérification",
                          ...
                          total_price_cent_incl_vat: calculated_total_price_cent_incl_vat,
      ...
      if @order.valid?
          ...
          cart_infos["items"].each do |item|
            ...
            unless product.nil?
              new_order_product = OrderProduct.create!(order_id: @order.id,
                                                      ...
                                                      product_price_cent_incl_vat: product.price_cent_incl_vat,
                                                      product_vat_rate: product.vat_rate)
              # On recalcule le prix total pour chaque item
              calculated_total_price_cent_incl_vat += product.price_cent_incl_vat * item["product_quantity"].to_i
            else
        ...
        total_price = calculated_total_price_cent_incl_vat + price_of_reservation

        # Si ils correspondent, on met a jour l'order et on passe à la suite
        if total_price == cart_infos["total_price"]
          @order.update(status: "En attente", total_price_cent_incl_vat: total_price)
